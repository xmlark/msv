...
public final class WSDLSchemaReader {
	private static final class SimpleNamespaceContext implements
			NamespaceContext {
	 ...
	 * 
	 * @param wsdlSource
	 *            the WSDL, in any of the TRaX sources.
	 * @param factory
	 *            a SAX parser factory, used to obtain a SAX parser used
	 *            internally in the reading process.
	 * @param controller
	 *            Object to handle errors, warnings, and provide a resolver for
	 *            non-local schemas.
	...
	public static XMLSchemaGrammar read(Source wsdlSource,
			SAXParserFactory factory, GrammarReaderController2 controller)
		...
		NodeList schemaNodes = (NodeList) xpath.evaluate("//xs:schema", wsdl,
		...
		for (int x = 0; x < schemaNodes.getLength(); x++) {
			...
			EmbeddedSchema embeddedWSDLSchema = new EmbeddedSchema(systemId,
					schema);
