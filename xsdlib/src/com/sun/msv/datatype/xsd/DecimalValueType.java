/*
 * @(#)DecimalValueType.java	1.29 00/02/02
 *
...
/**
 * Immutable, arbitrary-precision signed decimal numbers.  A DecimalValueType
 ...
 * DecimalValueType is <tt>(unscaledValue/10<sup>scale</sup>)</tt>.  DecimalValueType
 * provides operations for basic arithmetic, scale manipulation, comparison,
 * hashing, and format conversion.
 * <p>
 * The DecimalValueType class gives its user complete control over rounding
 ...
 * DecimalValueType: scaling/rounding operations and decimal point motion
 * operations.  Scaling/rounding operations (<tt>SetScale</tt>) return a
 * DecimalValueType whose value is approximately (or exactly) equal to that of the
 ...
 * <tt>movePointRight</tt>) return a DecimalValueType created from the operand by
 ...
 * descriptions of DecimalValueType methods.  The pseudo-code expression
 * <tt>(i + j)</tt> is shorthand for "a DecimalValueType whose value is
 * that of the DecimalValueType <tt>i</tt> plus that of the DecimalValueType <tt>j</tt>."
 * The pseudo-code expression <tt>(i == j)</tt> is shorthand for
 * "<tt>true</tt> if and only if the DecimalValueType <tt>i</tt> represents the same
 * value as the the DecimalValueType <tt>j</tt>."  Other pseudo-code expressions are
 * interpreted similarly. 
 * <p>
 * Note: care should be exercised if BigDecimals are to be used as keys in a
 * SortedMap or elements in a SortedSet, as DecimalValueType's <i>natural
 ...
 * @author Josh Bloch
 */
public class DecimalValueType extends Number implements Comparable {
    /**
     * The unscaled value of this DecimalValueType, as returned by unscaledValue().
     *
     * @serial
     * @see #unscaledValue
     */
    ...
    /**
     * The scale of this DecimalValueType, as returned by scale().
     *
     * @serial
     * @see #scale
     */
    ...
    /**
     * Translates the String representation of a BigDecmal into a DecimalValueType.
     ...
     *
     * <p>The scale of the returned DecimalValueType will be the number of digits in
     ...
     * negative, the scale of the returned DecimalValueType is zero and the unscaled
     * value is multiplied by the appropriate power of ten so that, in every
     * case, the resulting DecimalValueType is equal to <i>significand *
     ...
     * DecimalValueType, as it doesn't suffer from the unpredictability of the
     * {@link #DecimalValueType(double)} constructor.
     *
     * <p>Note: the optional leading plus sign and trailing exponent were
     * added in release 1.3.
     *
     * @param val String representation of DecimalValueType.
     * @throws NumberFormatException <tt>val</tt> is not a valid representation
     *	       of a DecimalValueType.
     */
    public DecimalValueType(String val) {
    ...
    /**
     * Translates a double into a DecimalValueType.  The scale of the DecimalValueType
     ...
     * One might assume that <tt>new DecimalValueType(.1)</tt> is exactly equal
     ...
     * <tt>new DecimalValueType(".1")</tt> is <i>exactly</i> equal to .1, as one
     * would expect.  Therefore, it is generally recommended that the (String)
     * constructor be used in preference to this one.
     *
     * @param val double value to be converted to DecimalValueType.
     ...
     */
    public DecimalValueType(double val) {
...
//	    intVal = BigInteger.ZERO;
    ...
    /**
     * Translates a BigInteger into a DecimalValueType.  The scale of the DecimalValueType
     * is zero.
     *
     * @param val BigInteger value to be converted to DecimalValueType.
     */
    public DecimalValueType(BigInteger val) {
	intVal = val;
    }

    /**
     * Translates a BigInteger unscaled value and an int scale into a
     * DecimalValueType.  The value of the DecimalValueType is
     * <tt>(unscaledVal/10<sup>scale</sup>)</tt>.
     *
     * @param unscaledVal unscaled value of the DecimalValueType.
     * @param scale scale of the DecimalValueType.
     * @throws NumberFormatException scale is negative
     */
    public DecimalValueType(BigInteger unscaledVal, int scale) {
	if (scale < 0)
    ...
    /**
     * Translates a long unscaled value and an int scale into a DecimalValueType.
     ...
     * @param unscaledVal unscaled value of the DecimalValueType.
     * @param scale scale of the DecimalValueType.
     * @return a DecimalValueType whose value is
     *	       <tt>(unscaledVal/10<sup>scale</sup>)</tt>.
     */
    public static DecimalValueType valueOf(long unscaledVal, int scale) {
	return new DecimalValueType(BigInteger.valueOf(unscaledVal), scale);
    }

    /**
     * Translates a long value into a DecimalValueType with a scale of zero.
     * This "static factory method" is provided in preference to a
     * (long) constructor because it allows for reuse of frequently
     * used BigDecimals.
     *
     * @param val value of the DecimalValueType.
     * @return a DecimalValueType whose value is <tt>val</tt>.
     */
    public static DecimalValueType valueOf(long val) {
    ...
    /**
     * Returns a DecimalValueType whose value is <tt>(this + val)</tt>, and whose
     * scale is <tt>max(this.scale(), val.scale())</tt>.
     *
     * @param  val value to be added to this DecimalValueType.
     * @return <tt>this + val</tt>
     */
    public DecimalValueType add(DecimalValueType val){
	DecimalValueType arg[] = new DecimalValueType[2];
	arg[0] = this;	arg[1] = val;
	matchScale(arg);
	return new DecimalValueType(arg[0].intVal.add(arg[1].intVal), arg[0].scale);
    }
     ...
     * scale is <tt>max(this.scale(), val.scale())</tt>.
     *
     * @param  val value to be subtracted from this DecimalValueType.
     * @return <tt>this - val</tt>
     */
    public DecimalValueType subtract(DecimalValueType val){
	DecimalValueType arg[] = new DecimalValueType[2];
	arg[0] = this;	arg[1] = val;
	matchScale(arg);
	return new DecimalValueType(arg[0].intVal.subtract(arg[1].intVal),
			      arg[0].scale);
    }
     ...
     *
     * @param  val value to be multiplied by this DecimalValueType.
     * @return <tt>this * val</tt>
     */
    public DecimalValueType multiply(DecimalValueType val){
	return new DecimalValueType(intVal.multiply(val.intVal), scale+val.scale);
    }
     ...
     * applied.
     *
     * @param  val value by which this DecimalValueType is to be divided.
     * @param  scale scale of the DecimalValueType quotient to be returned.
     ...
     */
    public DecimalValueType divide(DecimalValueType val, int scale, int roundingMode) {
	if (scale < 0)
	 ...
	 */
	DecimalValueType dividend, divisor;
	...
	if (r.signum() == 0)
	    return new DecimalValueType(q, scale);
	...
	return (increment
		? new DecimalValueType(q.add(BigInteger.valueOf(signum)), scale)
		: new DecimalValueType(q, scale));
    }
     ...
     * @see    #ROUND_UNNECESSARY
     */
    public DecimalValueType divide(DecimalValueType val, int roundingMode) {
	    return this.divide(val, scale, roundingMode);
    }

   /**
    * Returns a DecimalValueType whose value is the absolute value of this
    * DecimalValueType, and whose scale is <tt>this.scale()</tt>.
    *
    * @return <tt>abs(this)</tt>
    */
    public DecimalValueType abs(){
	return (signum() < 0 ? negate() : this);
    }

    /**
     * Returns a DecimalValueType whose value is <tt>(-this)</tt>, and whose scale
     * is this.scale().
     *
     * @return <tt>-this</tt>
     */
    public DecimalValueType negate(){
	return new DecimalValueType(intVal.negate(), scale);
    }

    /**
     * Returns the signum function of this DecimalValueType.
     *
     * @return -1, 0 or 1 as the value of this DecimalValueType is negative, zero or
     *	       positive.
     */
    public int signum(){
    ...
    /**
     * Returns the <i>scale</i> of this DecimalValueType.  (The scale is the number
     * of digits to the right of the decimal point.)
     *
     * @return the scale of this DecimalValueType.
     */
    public int scale() {
	return scale;
    }
     ...
     * DecimalValueType.  (Computes <tt>(this * 10<sup>this.scale()</sup>)</tt>.)
     *
     * @return the unscaled value of this DecimalValueType.
     * @since   1.2
     */
     ...
     * DecimalValueType is positive, behaves as for <tt>ROUND_UP</tt>; if negative, 
     ...
     * DecimalValueType is positive, behave as for <tt>ROUND_DOWN</tt>; if negative,
    ...
    /**
     * Returns a DecimalValueType whose scale is the specified value, and whose
     * unscaled value is determined by multiplying or dividing this
     * DecimalValueType's unscaled value by the appropriate power of ten to maintain
     ...
     * the division.
     *
     * @param  scale scale of the DecimalValueType value to be returned.
     * @param  roundingMode The rounding mode to apply.
     * @return a DecimalValueType whose scale is the specified value, and whose
     *	       unscaled value is determined by multiplying or dividing this
     * 	       DecimalValueType's unscaled value by the appropriate power of ten to
     ...
     * @see    #ROUND_UNNECESSARY
     */
    public DecimalValueType setScale(int scale, int roundingMode) {
	if (scale < 0)
	    throw new ArithmeticException("Negative scale");
	if (roundingMode < ROUND_UP || roundingMode > ROUND_UNNECESSARY)
	    throw new IllegalArgumentException("Invalid rounding mode");
	...
	if (scale == this.scale)
	    ...
	    return new DecimalValueType(timesTenToThe(intVal, scale-this.scale),
				  scale);
    ...
    /**
     * Returns a DecimalValueType whose scale is the specified value, and whose
     * value is numerically equal to this DecimalValueType's.  Throws an
     ...
     * exists a DecimalValueType of the specified scale and the correct value.  The
     * call can also be used to reduce the scale if the caller knows that the
     * DecimalValueType has sufficiently many zeros at the end of its fractional
     ...
     */
    public DecimalValueType setScale(int scale) {
    ...
    /**
     * Returns a DecimalValueType which is equivalent to this one with the decimal
     ...
     * movePointRight(-n).  (The DecimalValueType returned by this call has value
     * <tt>(this * 10<sup>-n</sup>)</tt> and scale
     * <tt>max(this.scale()+n, 0)</tt>.)
     *
     * @param  n number of places to move the decimal point to the left.
     * @return a DecimalValueType which is equivalent to this one with the decimal
     *	       point moved <tt>n</tt> places to the left.
     */
    public DecimalValueType movePointLeft(int n){
	return (n>=0 ? new DecimalValueType(intVal, scale+n) : movePointRight(-n));
    }

    /**

     * Moves the decimal point the specified number of places to the right.
     * If this DecimalValueType's scale is &gt;= <tt>n</tt>, the call merely
     ...
     * DecimalValueType returned by this call has value
     * <tt>(this * 10<sup>n</sup>)</tt> and scale
     * <tt>max(this.scale()-n, 0)</tt>.)
     *
     ...
     */
    public DecimalValueType movePointRight(int n){
	return (scale >= n ? new DecimalValueType(intVal, scale-n)
		           : new DecimalValueType(timesTenToThe(intVal, n-scale),0));
    }

    // Comparison Operations

    /**
     * Compares this DecimalValueType with the specified DecimalValueType.   Two
     ...
     *
     * @param  val DecimalValueType to which this DecimalValueType is to be compared.
     * @return -1, 0 or 1 as this DecimalValueType is numerically less than, equal
     *         to, or greater than <tt>val</tt>.
     */
    public int compareTo(DecimalValueType val){
    ...
    /**
     * Compares this DecimalValueType with the specified Object.  If the Object is a
     * DecimalValueType, this method behaves like <tt>compareTo(DecimalValueType)</tt>.
     * Otherwise, it throws a <tt>ClassCastException</tt> (as BigDecimals are
     * comparable only to other BigDecimals).
     *
     * @param  o Object to which this DecimalValueType is to be compared.
     * @return a negative number, zero, or a positive number as this
     *	       DecimalValueType is numerically less than, equal to, or greater
     *	       than <tt>o</tt>, which must be a DecimalValueType.
     * @throws ClassCastException <tt>o</tt> is not a DecimalValueType.
     * @see    #compareTo(java.math.DecimalValueType)
     * @see    Comparable
     * @since  1.2
     */
    public int compareTo(Object o) {
	return compareTo((DecimalValueType)o);
    }

    /**
     * Compares this DecimalValueType with the specified Object for equality.
     ...
     *
     * @param  x Object to which this DecimalValueType is to be compared.
     * @return <tt>true</tt> if and only if the specified Object is a
     *	       DecimalValueType whose value and scale are equal to this DecimalValueType's.
     * @see    #compareTo(java.math.DecimalValueType)
     */
    public boolean equals(Object x){
	if (!(x instanceof DecimalValueType))
	    return false;
	DecimalValueType xDec = (DecimalValueType) x;

	return scale == xDec.scale && intVal.equals(xDec.intVal);
    }

    /**
     * Returns the minimum of this DecimalValueType and <tt>val</tt>.
     *
     * @param  val value with with the minimum is to be computed.
     * @return the DecimalValueType whose value is the lesser of this DecimalValueType and 
     *	       <tt>val</tt>.  If they are equal, as defined by the
     * 	       <tt>compareTo</tt> method, either may be returned.
     * @see    #compareTo(java.math.DecimalValueType)
     */
    public DecimalValueType min(DecimalValueType val){
	return (compareTo(val)<0 ? this : val);
    }

    /**
     * Returns the maximum of this DecimalValueType and <tt>val</tt>.
     *
     * @param  val value with with the maximum is to be computed.
     * @return the DecimalValueType whose value is the greater of this DecimalValueType
     *	       and <tt>val</tt>.  If they are equal, as defined by the
     * 	       <tt>compareTo</tt> method, either may be returned.
     * @see    #compareTo(java.math.DecimalValueType)
     */
    public DecimalValueType max(DecimalValueType val){
	return (compareTo(val)>0 ? this : val);
    }
    ...
    /**
     * Returns the hash code for this DecimalValueType.  Note that two BigDecimals
     * that are numerically equal but differ in scale (like 2.0 and 2.00)
     * will generally <i>not</i> have the same hash code.
     *
     * @return hash code for this DecimalValueType.
     */
    public int hashCode() {
    ...
    /**
     * Returns the string representation of this DecimalValueType.  The digit-to-
     ...
     *
     * @return String representation of this DecimalValueType.
     * @see    Character#forDigit
     * @see    #DecimalValueType(java.lang.String)
     */
    ...
    /**
     * Converts this DecimalValueType to a BigInteger.  Standard <i>narrowing
     * primitive conversion</i> as defined in <i>The Java Language
     * Specification</i>: any fractional part of this DecimalValueType will be
     * discarded.
     *
     * @return this DecimalValueType converted to a BigInteger.
     */
    ...
    /**
     * Converts this DecimalValueType to an int.  Standard <i>narrowing primitive
     * conversion</i> as defined in <i>The Java Language Specification</i>:
     * any fractional part of this DecimalValueType will be discarded, and if the
     * resulting "BigInteger" is too big to fit in an int, only the low-order
     * 32 bits are returned.
     * 
     * @return this DecimalValueType converted to an int.
     */
    public int intValue(){
    ...
    /**
     * Converts this DecimalValueType to a long.  Standard <i>narrowing primitive
     * conversion</i> as defined in <i>The Java Language Specification</i>:
     * any fractional part of this DecimalValueType will be discarded, and if the
    ...
    /**
     * Converts this DecimalValueType to a float.  Similar to the double-to-float
     * <i>narrowing primitive conversion</i> defined in <i>The Java Language
     * Specification</i>: if this DecimalValueType has too great a magnitude to
     * represent as a float, it will be converted to
     * <tt>FLOAT.NEGATIVE_INFINITY</tt> or <tt>FLOAT.POSITIVE_INFINITY</tt>
     * as appropriate.
     * 
     * @return this DecimalValueType converted to a float.
     */
    ...
    /**
     * Converts this DecimalValueType to a double.  Similar to the double-to-float
     * <i>narrowing primitive conversion</i> defined in <i>The Java Language
     * Specification</i>: if this DecimalValueType has too great a magnitude to
     ...
     * as appropriate.
     * 
     * @return this DecimalValueType converted to a double.
     */
     ...
     * the lower-scaled DecimalValueType so they match.
     */
    private static void matchScale(DecimalValueType[] val) {
    ...
    /**
     * Reconstitute the <tt>DecimalValueType</tt> instance from a stream (that is,
     * deserialize it).
     */
	...
	s.defaultReadObject();
	    ...
	    throw new java.io.StreamCorruptedException(
                                      "DecimalValueType: Negative scale");
    }
}
